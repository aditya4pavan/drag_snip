...
export default function TableView() {
    const [det, red, range] = useSelector(state => [state.detections, state.color, state.range]);
    ...
    const data = output.filter(e => {
        if (e.label === 'stop') {
            const [light, dark] = range
            return e.percent >= (100 + light) && e.percent <= (100 + dark)
        }
        else
            return true
    }).map((e, i) => {
